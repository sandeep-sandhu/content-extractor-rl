...
fn benchmark_text_quality_calculation(c: &mut Criterion) {
    ...
    c.bench_function("text_quality_calculation", |b| {
        b.iter(|| {
            content_extractor_rl::text_utils::TextUtils::calculate_text_quality(
                black_box(text),
...
fn benchmark_html_parsing(c: &mut Criterion) {
    ...
    c.bench_function("html_parsing", |b| {
        b.iter(|| {
            content_extractor_rl::html_parser::HtmlParser::parse(black_box(&html)).unwrap()
        });
    });
}
...
fn benchmark_reward_calculation(c: &mut Criterion) {
    let config = Config::default();
    let calculator = content_extractor_rl::reward::ImprovedRewardCalculator::new(config.stopwords);
...
fn benchmark_replay_buffer_operations(c: &mut Criterion) {
    let mut buffer = content_extractor_rl::replay_buffer::PrioritizedReplayBuffer::new(10000, 0.6, 0.4);
...
fn benchmark_curriculum_difficulty_estimation(c: &mut Criterion) {
    let curriculum = content_extractor_rl::curriculum::CurriculumManager::new();
    let html = create_sample_html(20);
