...
pub struct DQNAgent {
    ...
    optimizer: AdamW,
    varmap: VarMap, // Store VarMap for accessing variables
    num_actions: usize,
    num_params: usize,
    gamma: f32,
    step_count: usize,
...
impl DQNAgent {
    ...
    pub fn new(
        ...
        lr: f64,
        device: &Device,
        varmap: VarMap, // Changed from VarBuilder to VarMap
    ) -> Result<Self> {
        // Create VarBuilder from the VarMap
        let vb = VarBuilder::from_varmap(&varmap, DType::F32, device);
        let online_network = DuelingDQN::new(state_dim, num_actions, num_params, vb.pp("online"))?;

        // Create target network with separate VarMap
